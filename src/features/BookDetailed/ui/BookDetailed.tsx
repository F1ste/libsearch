...
export const BookDetailed = () => {
	...
	const { data: book, isFetching, isError } = bookAPI.useFetchSingleBookQuery(urlParams?.book ?? '');
	...
	if (isError) {
		return (
			<div className={style.bookDetailed__error}>Произошла ошибка при получении данных</div>
		)
	}
	...
	return (
		<div className={style.bookDetailed}>
			...
			<div className={style.bookDetailed__bookInfo}>
				...
				<div className={style.bookDetailed__infoItem}>
					<div>Описание:</div>
					<div className={style.bookDetailed__description} dangerouslySetInnerHTML={{ __html: description ?? '' }} />
				</div>
